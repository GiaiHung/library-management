...
public class BorrowingSaga {
  @Setter private transient CommandGateway commandGateway;

  @Setter private transient QueryGateway queryGateway;
  ...
  private void handle(CreateBorrowingEvent event) {
    log.info(
        "CreateBorrowingEvent saga started from employee id: {} - for book id: {}",
        event.getEmployeeId(),
        event.getId());
    ...
    try {
      ...
      BookResponseModel response =
          queryGateway.query(query, ResponseTypes.instanceOf(BookResponseModel.class)).join();
      if (!response.isReady()) {
        throw new Exception("Book already borrowed");
      }

      SagaLifecycle.associateWith("bookId", event.getBookId());
      UpdateBookStatusCommand command =
  ...
  private void handle(BookStatusUpdatedEvent event) {
    ...
    try {
      ...
      EmployeeDTO employee =
          queryGateway.query(query, ResponseTypes.instanceOf(EmployeeDTO.class)).join();
  ...
  private void rollbackUpdateBookStatus(String bookId, String employeeId, String borrowingId) {
    ...
    RollbackBookStatusCommand command =
        new RollbackBookStatusCommand(bookId, true, employeeId, borrowingId);
    commandGateway.send(command);
  }
